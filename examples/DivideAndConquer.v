...
(*
Section DivideAndConquer_def.
Import DivideAndConquer.
Context `{DivideAndConquer}.

Function dc_solve (d:D) {wf smaller d} : R :=
  if primitive d then
    direct_solve d
  else
    let (d1, d2) := decompose d in
    compose (dc_solve d1) (dc_solve d2).
intros.
...
Spec Interpretation dc_impl : DivideAndConquer -> DivideAndConquer_impl.
prove_simple_interp {{ }}.
rewrite pf8. intros.
apply (DivideAndConquer_impl.solve_correct (solve__param:=t7) (solve__proof__param:=pf8) (solve_soundness__param:=axiom0) (direct_solve_correct__param:=axiom)).
Defined.
